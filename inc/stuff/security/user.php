  ...
  if ($_sec_user_included_ != '#sec_user_Included#') {
    ...
    function user_authorized () {
      ...
      db_query ('SELECT * FROM `user` WHERE (`authorized`=1) AND (`login`="'.
                $_SESSION['user_login'].'") AND (`password`=MD5("'.
                addslashes (user_password_hash ($_SESSION['user_login'], $_SESSION['user_password'])).
                '"))');
    ...
    function user_access_root () {
      return $_SESSION['user_access'] >= ACCESS_ROOT;
    }
    ...
    function user_login () { return $_SESSION['user_login']; }
